...
/*https://learndb.ru/courses/task/111
...
SELECT
  ...
  , array[ROW_NUMBER() OVER (ORDER BY first_name, last_name, employee_id)] AS sort  
FROM employee
WHERE manager_id IS NULL
...
SELECT
  ...
  , CONCAT(LPAD('', count_managers * 8, ' '), CONCAT(e.first_name, ' ', e.last_name)) AS full_name
  , sort || ROW_NUMBER() 
              OVER (PARTITION BY e.manager_id ORDER BY e.first_name, e.last_name, e.employee_id) AS sort  
FROM hierarchy h
  INNER JOIN employee e
    ON e.manager_id = h.employee_id
)
SELECT
      employee_id
    , full_name
    , count_managers < LEAD(count_managers, 1, count_managers) OVER (ORDER BY sort) AS is_manager
FROM hierarchy
ORDER BY sort
